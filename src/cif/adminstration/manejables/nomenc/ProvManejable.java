...
import cif.gui.nomencladores.ProvNomencPanel;
 ...
 * @author Pedro
 */
public class ProvManejable extends NomencManejable
{
    private static final int NAME = 0;
    public ProvManejable(JF_Principal princ)
    {
        super(princ);
    }
    ...
    public void Insert()
    {
        ProvNomencPanel mp = new ProvNomencPanel();
    ...
    public void Modify()
    {
        if(Utils.isTable(parent.getPrincipalComponent()))
        {
            ...
            ProvNomencPanel mp = new ProvNomencPanel(name);
            parent.setComponent(mp);
        }
    }
    ...
    public void Delete()
    {
        ...
        selected_option=JOptionPane.showInternalConfirmDialog(parent.getContentPane(), "Esta acción eliminará los datos relacionados con la Provincia seleccionado.\n\rDesea continuar?", "Advertencia", JOptionPane.YES_NO_OPTION,JOptionPane.WARNING_MESSAGE);
        if(selected_option != JOptionPane.OK_OPTION)
        {
            return;
        }
        ...
        {
            id=DBQueries.getNomencId("provincias",name[i]);
            DBQueries.UpdateNomenc("provincias",id,name[i],"false");
    ...
    public JTable Report() {
        return DBQueries.getAllNomenc("provincias");
    }
