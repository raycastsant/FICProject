 ...
 * @author Pedro
 */
public class TalasNomencPanel extends javax.swing.JPanel {
    ...
    public TalasNomencPanel()
    {
        initComponents();
        action = INSERT;
    }

    public TalasNomencPanel(String nombre)
    {
        initComponents();
        action = UPDATE;
        old_name = nombre;
        name_field.setText(nombre);
        ff_id = DBQueries.getNomencId("c4_3_talas",nombre);
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Tipo de Tala"));
    ...
    private void InsertFF()
    {
        ...
        if(error)
        {
            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"Debe ponerle un nombre al Tipo de tala.", "Error", JOptionPane.ERROR_MESSAGE);
            error = false;
            return;
        }
        else
        {
            try
            {
                if(action == INSERT)
                {
                    if(!DBQueries.ExistNomenc("c4_3_talas",name))
                    {
                        DBQueries.InsertNomenc("c4_3_talas",name);
                    }
                    else
                    {
                        ff_id = DBQueries.getNomencId("c4_3_talas",name);
                        if(!BDUtilities.isActive("c4_3_talas", ff_id, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c4_3_talas",ff_id,name,"true");
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El tipo de tala \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
                    ResetWindow();
                ...
                else
                {
                    ...
                    {
                        String aux = DBQueries.getNomencId("c4_3_talas",name);
                        if(old_name.equals(name))
                        {
                            DBQueries.UpdateNomenc("c4_3_talas",ff_id,name,"true");
                        }
                        if(!BDUtilities.isActive("c4_3_talas", aux, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c4_3_talas",ff_id,name,"true");
                            BDUtilities.deleteNomenc("c4_3_talas", aux, FieldDescriptor.NUMBER);
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El tipo de tala \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
