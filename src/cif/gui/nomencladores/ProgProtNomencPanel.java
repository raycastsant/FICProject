 ...
 * @author Pedro
 */
public class ProgProtNomencPanel extends javax.swing.JPanel {
    ...
    public ProgProtNomencPanel()
    {
        initComponents();
        action = INSERT;
    }

    public ProgProtNomencPanel(String nombre)
    {
        initComponents();
        action = UPDATE;
        old_name = nombre;
        name_field.setText(nombre);
        ff_id = DBQueries.getNomencId("c3_1_3_ejecprogprotec",nombre);
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Programa de Protección"));
    ...
    private void InsertFF()
    {
        ...
        if(error)
        {
            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"Debe ponerle un nombre al Programa de Protección.", "Error", JOptionPane.ERROR_MESSAGE);
            error = false;
            return;
        }
        else
        {
            try
            {
                if(action == INSERT)
                {
                    if(!DBQueries.ExistNomenc("c3_1_3_ejecprogprotec",name))
                    {
                        DBQueries.InsertNomenc("c3_1_3_ejecprogprotec",name);
                    }
                    else
                    {
                        ff_id = DBQueries.getNomencId("c3_1_3_ejecprogprotec",name);
                        if(!BDUtilities.isActive("c3_1_3_ejecprogprotec", ff_id, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c3_1_3_ejecprogprotec",ff_id,name,"true");
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El Programa de Protección \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
                    ResetWindow();
                ...
                else
                {
                    ...
                    {
                        String aux = DBQueries.getNomencId("c3_1_3_ejecprogprotec",name);
                        if(old_name.equals(name))
                        {
                            DBQueries.UpdateNomenc("c3_1_3_ejecprogprotec",ff_id,name,"true");
                        }
                        if(!BDUtilities.isActive("c3_1_3_ejecprogprotec", aux, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c3_1_3_ejecprogprotec",ff_id,name,"true");
                            BDUtilities.deleteNomenc("c3_1_3_ejecprogprotec", aux, FieldDescriptor.NUMBER);
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El Programa de Protección \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
