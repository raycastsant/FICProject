 ...
 * @author Pedro
 */
public class ProgEducNomencPanel extends javax.swing.JPanel {

    private static final int INSERT = 0;
    private static final int UPDATE = 1;
    private int action;
    private String ff_id,old_peduc;
    /** Creates new form ProvNomencPanel */
    public ProgEducNomencPanel()
    {
        initComponents();
        action = INSERT;
    }

    public ProgEducNomencPanel(String nombre)
    {
        initComponents();
        action = UPDATE;
        old_peduc = nombre;
        name_field.setText(nombre);
        ff_id = DBQueries.getNomencId("c3_1_4_progeducext",nombre);
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Programa Educativo"));
    ...
    private void InsertFF()
    {
        ...
        if(error)
        {
            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"Debe ponerle un nombre al Programa Educativo.", "Error", JOptionPane.ERROR_MESSAGE);
            error = false;
            return;
        }
        else
        {
            try
            {
                if(action == INSERT)
                {
                    if(!DBQueries.ExistNomenc("c3_1_4_progeducext",name))
                    {
                        DBQueries.InsertNomenc("c3_1_4_progeducext",name);
                    }
                    else
                    {
                        ff_id = DBQueries.getNomencId("c3_1_4_progeducext",name);
                        if(!BDUtilities.isActive("c3_1_4_progeducext", ff_id, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c3_1_4_progeducext",ff_id,name,"true");
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El Programa Educativo \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
                    ResetWindow();
                ...
                else
                {
                    ...
                    {
                        String aux = DBQueries.getNomencId("c3_1_4_progeducext",name);
                        if(old_peduc.equals(name))
                        {
                            DBQueries.UpdateNomenc("c3_1_4_progeducext",ff_id,name,"true");
                        }
                        else
                        {
                            if(!BDUtilities.isActive("c3_1_4_progeducext", aux, FieldDescriptor.STRING))
                            {
                                DBQueries.UpdateNomenc("c3_1_4_progeducext",ff_id,name,"true");
                                BDUtilities.deleteNomenc("c3_1_4_progeducext", aux, FieldDescriptor.NUMBER);
                            }
                            else
                            {
                                JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El Programa Educativo \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
