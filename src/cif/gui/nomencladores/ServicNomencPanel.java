 ...
 * @author Pedro
 */
public class ServicNomencPanel extends javax.swing.JPanel {
    ...
    public ServicNomencPanel()
    {
        initComponents();
        action = INSERT;
    }

    public ServicNomencPanel(String nombre)
    {
        initComponents();
        action = UPDATE;
        old_name = nombre;
        name_field.setText(nombre);
        ff_id = DBQueries.getNomencId("c5_4_servicios",nombre);
    }
    ...
    private void InsertFF()
    {
        ...
        else
        {
            try
            {
                if(action == INSERT)
                {
                    if(!DBQueries.ExistNomenc("c5_4_servicios",name))
                    {
                        DBQueries.InsertNomenc("c5_4_servicios",name);
                    }
                    else
                    {
                        ff_id = DBQueries.getNomencId("c5_4_servicios",name);
                        if(!BDUtilities.isActive("c5_4_servicios", ff_id, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c5_4_servicios",ff_id,name,"true");
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El servicio \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
                    ResetWindow();
                ...
                else
                {
                    ...
                    {
                        String aux = DBQueries.getNomencId("c5_4_servicios",name);
                        if(old_name.equals(name))
                        {
                            DBQueries.UpdateNomenc("c5_4_servicios",ff_id,name,"true");
                        }
                        if(!BDUtilities.isActive("c5_4_servicios", aux, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c5_4_servicios",ff_id,name,"true");
                            BDUtilities.deleteNomenc("c5_4_servicios", aux, FieldDescriptor.NUMBER);
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El servicio \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
