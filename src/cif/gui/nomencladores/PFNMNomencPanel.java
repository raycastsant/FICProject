 ...
 * @author Pedro
 */
public class PFNMNomencPanel extends javax.swing.JPanel {
    ...
    public PFNMNomencPanel()
    {
        initComponents();
        action = INSERT;
    }

    public PFNMNomencPanel(String nombre)
    {
        initComponents();
        action = UPDATE;
        old_name = nombre;
        name_field.setText(nombre);
        ff_id = DBQueries.getNomencId("c4_4_prodfornomad",nombre);
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Producto Forestal no Maderable"));
    ...
    private void InsertFF()
    {
        ...
        if(error)
        {
            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"Debe ponerle un nombre al producto.", "Error", JOptionPane.ERROR_MESSAGE);
            error = false;
            return;
        }
        else
        {
            try
            {
                if(action == INSERT)
                {
                    if(!DBQueries.ExistNomenc("c4_4_prodfornomad",name))
                    {
                        DBQueries.InsertNomenc("c4_4_prodfornomad",name);
                    }
                    else
                    {
                        ff_id = DBQueries.getNomencId("c4_4_prodfornomad",name);
                        if(!BDUtilities.isActive("c4_4_prodfornomad", ff_id, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c4_4_prodfornomad",ff_id,name,"true");
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El producto \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
                    ResetWindow();
                ...
                else
                {
                    ...
                    {
                        String aux = DBQueries.getNomencId("c4_4_prodfornomad",name);
                        if(old_name.equals(name))
                        {
                            DBQueries.UpdateNomenc("c4_4_prodfornomad",ff_id,name,"true");
                        }
                        if(!BDUtilities.isActive("c4_4_prodfornomad", aux, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c4_4_prodfornomad",ff_id,name,"true");
                            BDUtilities.deleteNomenc("c4_4_prodfornomad", aux, FieldDescriptor.NUMBER);
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"El producto \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
