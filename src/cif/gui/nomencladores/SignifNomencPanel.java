 ...
 * @author Pedro
 */
public class SignifNomencPanel extends javax.swing.JPanel {
    ...
    public SignifNomencPanel()
    {
        initComponents();
        action = INSERT;
    }

    public SignifNomencPanel(String nombre)
    {
        initComponents();
        action = UPDATE;
        old_name = nombre;
        name_field.setText(nombre);
        ff_id = DBQueries.getNomencId("significacion",nombre);
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Significación"));
    ...
    private void InsertFF()
    {
        ...
        if(error)
        {
            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"Debe ponerle un nombre a la Significación.", "Error", JOptionPane.ERROR_MESSAGE);
            error = false;
            return;
        }
        else
        {
            try
            {
                if(action == INSERT)
                {
                    if(!DBQueries.ExistNomenc("significacion",name))
                    {
                        DBQueries.InsertNomenc("significacion",name);
                    }
                    else
                    {
                        ff_id = DBQueries.getNomencId("significacion",name);
                        if(!BDUtilities.isActive("significacion", ff_id, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("significacion",ff_id,name,"true");
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"La significación \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
                    ResetWindow();
                ...
                else
                {
                    ...
                    {
                        String aux = DBQueries.getNomencId("significacion",name);
                        if(old_name.equals(name))
                        {
                            DBQueries.UpdateNomenc("significacion",ff_id,name,"true");
                        }
                        if(!BDUtilities.isActive("significacion", aux, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("significacion",ff_id,name,"true");
                            BDUtilities.deleteNomenc("significacion", aux, FieldDescriptor.NUMBER);
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"La significación \""+name+ "\" ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
