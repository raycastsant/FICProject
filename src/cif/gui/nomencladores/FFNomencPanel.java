 ...
 * @author Pedro
 */
public class FFNomencPanel extends javax.swing.JPanel {
    ...
    public FFNomencPanel()
    {
        initComponents();
        action = INSERT;
    }

    public FFNomencPanel(String nombre)
    {
        initComponents();
        action = UPDATE;
        old_name = nombre;
        name_field.setText(nombre);
        ff_id = DBQueries.getNomencId("c3_1_1_formacionforestal",nombre);
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Formación Forestal"));
    ...
    private void InsertFF()
    {
        ...
        if(error)
        {
            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"Debe ponerle un nombre a la Formación Forestal.", "Error", JOptionPane.ERROR_MESSAGE);
            error = false;
            return;
        }
        else
        {
            try
            {
                if(action == INSERT)
                {
                    if(!DBQueries.ExistNomenc("c3_1_1_formacionforestal",name))
                    {
                        DBQueries.InsertNomenc("c3_1_1_formacionforestal",name);
                    }
                    else
                    {
                        ff_id = DBQueries.getNomencId("c3_1_1_formacionforestal",name);
                        if(!BDUtilities.isActive("c3_1_1_formacionforestal", ff_id, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c3_1_1_formacionforestal",ff_id,name,"true");
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"La formación forestal "+name+ " ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                    }
                    ResetWindow();
                ...
                else
                {
                    ...
                    {
                        String aux = DBQueries.getNomencId("c3_1_1_formacionforestal",name);
                        if(old_name.equals(name))
                        {
                            DBQueries.UpdateNomenc("c3_1_1_formacionforestal",ff_id,name,"true");
                        }
                        if(!BDUtilities.isActive("c3_1_1_formacionforestal", aux, FieldDescriptor.STRING))
                        {
                            DBQueries.UpdateNomenc("c3_1_1_formacionforestal",ff_id,name,"true");
                            BDUtilities.deleteNomenc("c3_1_1_formacionforestal", aux, FieldDescriptor.NUMBER);
                        }
                        else
                        {
                            JOptionPane.showInternalMessageDialog(Utils.getParentFrame(this).getContentPane(),"La formación forestal "+name+ " ya existe.", "Error", JOptionPane.ERROR_MESSAGE);
